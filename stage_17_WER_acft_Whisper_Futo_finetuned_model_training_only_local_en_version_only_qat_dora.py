...
"""
DORA:
https://chatgpt.com/g/g-p-6969433d33d4819187ec3158a8f3745f-whisper-training/c/6983d90c-713c-83a9-84ce-294f26b933bd
...
# PEFT/LoRA helpers (hardcoded config; no env vars required)
from stage17_main_utils_peft import (
    peft_is_enabled,
    maybe_wrap_with_lora_peft,
    load_whisper_student_with_optional_peft,
...
def inject_qat_linears(module: nn.Module, *, bits: int, state: QATState, prefix: str = ""):
    """Recursively wraps linear layers with FakeQuantLinear.

    Notes for PEFT (LoRA/DoRA):
    - We *avoid* quantizing the tiny LoRA matrices (A/B), because that's not what you'll ship.
    - We *do* quantize the frozen base linear that PEFT wraps (usually exposed as `.base_layer`).
    """
    for name, child in list(module.named_children()):
        full = f"{prefix}.{name}" if prefix else name

        # Never fake-quantise PEFT adapter internals (LoRA/DoRA).
        # We still want to quantise the *base* linear weights (frozen) for robustness,
        # but adapter weights should learn in FP.
        if ".lora_" in full or full.endswith(("lora_A","lora_B","lora_embedding_A","lora_embedding_B","lora_magnitude_vector")):
            continue

        # 1) Cheap exclusions (keep FP), by suffix
        if any(full.endswith(suf) for suf in QAT_EXCLUDE_SUFFIXES):
            continue

        # 2) Skip PEFT LoRA internals (A/B/magnitude/etc.). We'll quantize the base_layer instead.
        if ".lora_" in full or name.startswith("lora_"):
            continue
        ...
        # PEFT LoRA/DoRA wrappers: quantize the wrapped frozen base linear in-place.
        base_layer = getattr(child, "base_layer", None)
        if isinstance(base_layer, nn.Linear):
            child.base_layer = FakeQuantLinear(base_layer, bits=int(bits), state=state)
            continue
...
def load_student_from_checkpoint_or_base(model_dir: str | None):
    return load_whisper_student_with_optional_peft(
        model_dir=model_dir,
        base_model_id=PROCESSOR_ID,
        generation_config=gen_config,
        want_trainable_adapter=peft_is_enabled(),
    )
...
optimizer = torch.optim.AdamW(iter_trainable_params(model_train), lr=float(LR_START))
...
def _unwrap_whisper_for_conditional_generation(model):
    """Return base WhisperForConditionalGeneration even when wrapped by PEFT."""
...
def _unwrap_whisper_core(model):
    """Return WhisperModel (has encoder/decoder)."""
    base = _unwrap_whisper_for_conditional_generation(model)
    if hasattr(base, "model"):
        return base.model
    return base


FULL_ENCODER_CONTEXT_LENGTH = int(
    getattr(_unwrap_whisper_for_conditional_generation(model_train).config, "max_source_positions", 1500)
)
...
def train_one_epoch(epoch_num: int, loader):
    ...
    core_train = _unwrap_whisper_core(model_train)
    core_ref = _unwrap_whisper_core(model_ref) if want_acft else None
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        max_embed_positions = int(core_train.encoder.embed_positions.weight.shape[0])
        ...
        with torch.amp.autocast("cuda", enabled=use_amp, dtype=amp_dtype):
            enc_partial = compute_partially_encoder(core_train, input_features, int(n_ctx))
            dec_partial = core_train.decoder(
        ...
        if want_acft:
            ...
            with torch.no_grad(), torch.amp.autocast("cuda", enabled=False):
                enc_full = compute_partially_encoder(core_ref, input_features, int(FULL_ENCODER_CONTEXT_LENGTH))
                dec_full = core_ref.decoder(
                    input_ids=decoder_input_ids,
                    attention_mask=decoder_attention_mask,
